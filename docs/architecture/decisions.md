...
**Decision** Every user-facing story splits into a **UX-design item** (Figma frame + flow, signed off) and a **build item**; the build item is not DoR-ready until the design is `✅` and its `ui-spec.md` section exists. Figma is the **visual source of truth**; `ui-spec.md` is the code-facing translation. Figma↔Claude handoff is the **official Figma MCP** (hosted plugin, `mcp.figma.com/mcp`), which is **bidirectional** — Claude can both read frames (→ implement SwiftUI) **and** generate/edit Figma designs via the write tools + `/figma-*` skills. Pipeline: `docs/process/design-workflow.md`.
**Consequences** Slower start per screen (design gate) but far less rework and a coherent design system. Designs may be **generated by Claude into Figma** or authored by Minh — PO signs off either way. Design tokens live in Figma variables **and** a Swift `DesignTokens` mirror — drift risk if not kept in sync. *(Supersedes the initial assumption that the MCP was read-only; that was the local Dev Mode server, since removed in favor of the hosted plugin.)*
